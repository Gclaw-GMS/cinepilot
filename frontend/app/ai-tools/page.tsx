...
export default function AIToolsPage() {
  ...
  const handleExportMarkdown = useCallback(() => {
    ...
    const categoryCounts = currentFilteredTools.reduce<Record<string, number>>((acc, tool) => {
...
|------|-------------|----------|
${currentFilteredTools.filter(t => t.category === category).map(t => `| ${t.name} | ${t.desc} | ${t.endpoint || 'N/A'} |`).join('\n')}
`).join('\n')}

---

*Generated by CinePilot AI Tools*
`
...
<body>
  ...
  // Get unique categories from tools for number key shortcuts
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Number keys 1-6 for category filtering (toggle behavior)
      if (e.key >= '1' && e.key <= '6') {
        e.preventDefault()
        const index = parseInt(e.key) - 1
        if (index < toolCategories.length) {
          const selectedCategory = toolCategories[index]
          // Toggle: if already selected, clear it; otherwise set it
          setCategoryFilterRef.current(prev => prev === selectedCategory ? 'all' : selectedCategory)
        } else if (index === toolCategories.length) {
          // Key 6 (or beyond) could be "all" if we have exactly 5 categories + 1 for "all"
          // For now, just clear to all if the index matches
          setCategoryFilterRef.current('all')
        }
  ...
  useEffect(() => {
    ...
    setCategoryFilterRef.current = setCategoryFilter
                  ...
                  {showFilterPanel && (
                    <div className="absolute right-0 mt-2 w-56 bg-slate-800 border border-slate-700 rounded-xl shadow-xl z-50 overflow-hidden">
                      <div className="p-4 border-b border-slate-700">
                        ...
                        >
                          <option value="all">All Categories ({toolCategories.length})</option>
                          {toolCategories.length > 0 ? toolCategories.map((cat, idx) => (
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-full max-w-md shadow-2xl">
              ...
              <div className="space-y-3">
                ...
                {toolCategories.map((cat, idx) => (
                  <div key={cat} className="flex items-center justify-between py-2 border-b border-slate-800">
                    <span className="text-slate-400">Filter: {cat}</span>
                    <kbd className="px-2 py-1 bg-slate-800 rounded text-sm font-mono">{idx + 1}</kbd>
                  </div>
                ))}
                <div className="flex items-center justify-between py-2 border-b border-slate-800">
                  <span className="text-slate-400">Clear filter (show all)</span>
                  <kbd className="px-2 py-1 bg-slate-800 rounded text-sm font-mono">{toolCategories.length}</kbd>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-slate-800">
