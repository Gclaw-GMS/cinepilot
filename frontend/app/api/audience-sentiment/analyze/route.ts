...
function generateDemoAnalysis(sentimentId: string) {
  let positiveCount = 0
  let negativeCount = 0
  let neutralCount = 0
  let totalScore = 0
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // If database is not connected, return demo analysis
    if (!dbConnected) {
      // Simulate processing time
      await new Promise(resolve => setTimeout(resolve, 1500))
      const demoResult = generateDemoAnalysis(sentimentId)
    ...
    return NextResponse.json({ sentiment: updatedSentiment, comments: analyzedComments, isDemo: false })
  } catch (error) {
    console.error('Error analyzing sentiment:', error)
    // Return demo analysis on error
    try {
      const body = await request.json()
      const demoResult = generateDemoAnalysis(body.sentimentId || 'demo-error')
      return NextResponse.json(demoResult)
    } catch {
