...
const DEMO_SHOTS: Record<string, unknown>[] = [
  ...
  { id: 'shot-12', shotIndex: 12, beatIndex: 2, shotText: 'Close-up of photo frame', characters: [], shotSize: 'CU', cameraAngle: 'high', cameraMovement: 'static', focalLengthMm: 85, lensType: 'prime', keyStyle: 'symbolic', colorTemp: '5600K', durationEstSec: 4, confidenceCamera: 0.85, confidenceLens: 0.9, confidenceLight: 0.8, confidenceDuration: 0.65, isLocked: false, userEdited: false, notes: 'Symbolic shot - photo of family', scene: { id: 'scene-3', sceneNumber: '3', headingRaw: 'INT. HOUSE - DAY', intExt: 'INT', timeOfDay: 'DAY', location: 'House' } },
]

// Deep freeze to prevent mutation
Object.freeze(DEMO_SHOTS)
DEMO_SHOTS.forEach(shot => Object.freeze(shot))

// In-memory storage for demo mode - always reset from DEMO_SHOTS to prevent mutation
function getDemoShots() {
  return JSON.parse(JSON.stringify(DEMO_SHOTS))
}
...
function getMutableDemoShots() {
  // Always get fresh data to prevent test pollution
  mutableDemoShots = getDemoShots()
  return mutableDemoShots
}

// Reset function for testing - clears mutable state (not exported, called internally)
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Always reset demo data to prevent mutation from previous requests
    const currentDemoShots = getDemoShots()
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'generateScript') {
      ...
      demoData.push(...generatedShots)
      
      return NextResponse.json({
        success: true,
        totalShots: demoData.length,
...
function generateDemoShots(sceneId: string): any[] {
  ...
  const demoData = getMutableDemoShots()!
  const existingCount = demoData.filter((s: any) => s.scene?.id === sceneId).length
  const baseIndex = demoData.length + 1
