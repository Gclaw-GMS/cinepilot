...
export default function CensorPage() {
  ...
  const handleExportRef = useRef<((format: 'json' | 'pdf' | 'markdown') => void) | null>(null)
  ...
  const handleExport = useCallback((format: 'json' | 'pdf' | 'markdown') => {
    if (!analysis) return
    ...
    if (format === 'json') {
      ...
      const sceneFlags = analysis.sceneFlags || []
      ...
      const markdown = `# 📊 Censor Certification Analysis
...
| Metric | Count |
|--------|-------|
| **Total Risk Flags** | ${sceneFlags.length} |
...
${Object.entries(categoryCounts).sort((a, b) => b[1] - a[1]).map(([cat, count]) => `- **${cat}**: ${count}`).join('\n')}
...
|-------|----------|----------|---------|
${sceneFlags.map(f => `| ${f.scene.sceneNumber} | ${f.category} | ${f.severity}/10 | ${f.context.substring(0, 50)}${f.context.length > 50 ? '...' : ''} |`).join('\n')}

---

## 💡 Suggestions

${suggestions.length > 0 ? suggestions.map(s => `### Scene ${s.sceneNumber}: ${s.issue}

- **Suggested Change:** ${s.suggestedChange}
- **Why:** ${s.why}
- **Severity Delta:** ${s.expectedSeverityDelta > 0 ? '+' : ''}${s.expectedSeverityDelta}

---`).join('\n') : '_No suggestions available_'}

---

## 🔍 Top Risk Drivers

${analysis.topDrivers.length > 0 ? analysis.topDrivers.map((driver, i) => `${i + 1}. ${driver}`).join('\n') : '_No drivers identified_'}

---

## ⚠️ High Risk Scenes

${analysis.highRiskScenes.length > 0 ? analysis.highRiskScenes.map(scene => `- Scene ${scene}`).join('\n') : '_No high risk scenes_'}

---

## ❓ Uncertainties

${uncertainties.length > 0 ? uncertainties.map(u => `- ${u}`).join('\n') : '_No uncertainties recorded_'}
...
<div class="footer">Generated by CinePilot • For reference only</div>
  ...
  }, [analysis, selectedProject, isDemoMode])
...
<body>
  ...
  // Update ref after handleExport is defined
  useEffect(() => {
          ...
          <div className="relative" ref={exportDropdownRef}>
            ...
            {showExportDropdown && (
              <div className="absolute right-0 top-full mt-2 w-48 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-50 overflow-hidden">
                ...
                <button
                  onClick={() => { handleExport('markdown'); setShowExportDropdown(false) }}
                  disabled={!analysis}
                  className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-700 transition-colors text-left disabled:opacity-50"
                >
