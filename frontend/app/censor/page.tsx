...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 40px; color: #1a1a2e; background }
    .header: #fff; { text-align: center; margin-bottom: 30px; border-bottom: 3px solid #dc2626; padding-bottom: 20px; }
...
<body>
  ...
  <div class="stats">
    <div class="stat"><div class="stat-value">${analysis.deterministicScore?.toFixed(1) || '0'}</div><div class="stat-label">Sensitivity Score</div></div>
    <div class="stat"><div class="stat-value">${analysis.sceneFlags.length}</div><div class="stat-label">Content Flags</div></div>
    <div class="stat"><div class="stat-value">${analysis.sceneFlags.filter(f => f.severity >= 4).length}</div><div class="stat-label">High Severity</div></div>
    <div class="stat"><div class="stat-value">${analysis.suggestions.length}</div><div class="stat-label">Suggestions</div></div>
  </div>
  <div class="section">
    <h2>📊 Content Flags (${analysis.sceneFlags.length} total)</h2>
    ${analysis.sceneFlags.map(flag => `
    <div class="flag">
      <div class="sev">${[1,2,3,4,5].map(i => `<div class="sev-dot ${i <= flag.severity ? 'active' : 'inactive'}"></div>`).join('')}</div>
  ...
  <div class="footer">Generated by CinePilot • Film Production Management</div>
</body>
</html>`
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {analysis && (
            <button onClick={() => handleExportPDF(analysis)} className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg font-medium text-sm transition-colors">
              <Download className="w-4 h-4" />
              Export PDF
            </button>
          )}
          <button onClick={handleAnalyze} disabled={analyzing} className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 disabled:opacity-50 rounded-lg font-medium text-sm transition-colors">
            {analyzing ? <><Loader2 className="w-4 h-4 animate-spin" />Analyzing...</> : <><Sparkles className="w-4 h-4" />Run Analysis</>}
          </button>
        </div>
      </div>
      ...
      {analysis && (
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className={`rounded-xl p-6 border text-center ${CERT_COLORS[analysis.predictedCertificate || 'U'] || 'bg-slate-900 border-slate-700'}`}>
            ...
            {analysis.confidence && <div className="text-xs mt-2 opacity-60 capitalize">Confidence: {analysis.confidence}</div>}
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 text-center">
            <div className="text-3xl font-bold text-orange-400">{analysis.sceneFlags.filter(f => f.severity >= 4).length}</div>
      ...
      <div className="flex gap-1 mb-6 border-b border-slate-800">
        {tabs.map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`px-5 py-3 text-sm font-medium rounded-t-lg transition-colors ${activeTab === tab.key ? 'bg-slate-900 text-indigo-400 border border-b-0 border-slate-800' : 'text-slate-500 hover:text-slate-300'}`}>
            {tab.label}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-6">
          {!analysis ? (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
              ...
              <button onClick={handleAnalyze} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-lg text-sm font-medium">Generate Demo Analysis</button>
            </div>
          ) : (
            <>
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                <h3 className="font-semibold mb-4 text-slate-300 flex items-center gap-2"><FileText className="w-4 h-4" />Category Breakdown</h3>
                <div className="grid grid-cols-3 gap-3">
                  {CATEGORIES.map(cat => (
                    <div key={cat} className="bg-slate-800/50 rounded-lg p-4 flex justify-between items-center">
                      <span className={`text-xs px-2 py-1 rounded ${CATEGORY_COLORS[cat]}`}>{cat.replace('_', ' ')}</span>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                <h3 className="font-semibold mb-3 text-slate-300 flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-amber-400" />Top Content Drivers</h3>
                <ul className="space-y-2">{analysis.topDrivers.map((d, i) => (<li key={i} className="text-sm text-slate-400 flex items-start gap-3"><span className="text-red-400 mt-0.5">•</span><span>{d}</span></li>))}</ul>
              </div>
              {analysis.uncertainties.length > 0 && (
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                  <h3 className="font-semibold mb-3 text-slate-300">Uncertainties</h3>
                  <ul className="space-y-2">{analysis.uncertainties.map((u, i) => (<li key={i} className="text-sm text-slate-500 flex items-start gap-2"><span className="text-slate-600">?</span><span>{u}</span></li>))}</ul>
                </div>
              )}
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                <h3 className="font-semibold mb-3 text-slate-300 flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-400" />Target Certificate Optimizer</h3>
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-slate-500">Target:</span>
                    <select value={targetCert} onChange={e => setTargetCert(e.target.value)} className="px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm">
                      {CERTIFICATES.map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                  </div>
                  <button onClick={handleSuggestCuts} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-lg text-sm font-medium transition-colors">Generate Cut Suggestions</button>
                </div>
                <p className="text-xs text-slate-500 mt-3">Get AI-powered suggestions to achieve your target certificate while preserving creative intent.</p>
              </div>
            </>
          )}
        </div>
      ...
      {activeTab === 'flags' && (
        <div className="space-y-2">
          {(!analysis || analysis.sceneFlags.length === 0) ? (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center text-slate-500">No flags found. Run analysis to detect content issues.</div>
          ) : (
            analysis.sceneFlags.map(flag => (
              <div key={flag.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center gap-4 hover:border-slate-700 transition-colors">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs px-2 py-1 rounded ${CATEGORY_COLORS[flag.category] || 'bg-slate-800 text-slate-500'}`}>{flag.category.replace('_', ' ')}</span>
                </div>
                ...
                {flag.context && <span className="text-xs text-slate-500 max-w-xs truncate">{flag.context}</span>}
              </div>
            ))
          )}
        </div>
      ...
      {activeTab === 'suggestions' && (
        <div className="space-y-3">
          {(!analysis || analysis.suggestions.length === 0) ? (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center text-slate-500">No suggestions yet. Run analysis and use the Target Optimizer to get cut suggestions.</div>
          ) : (
            analysis.suggestions.map(sug => (
              <div key={sug.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  {sug.effort && <span className={`text-[10px] px-2 py-1 rounded ${sug.effort === 'low' ? 'bg-green-900/30 text-green-400' : sug.effort === 'med' ? 'bg-yellow-900/30 text-yellow-400' : 'bg-red-900/30 text-red-400'}`}>Effort: {sug.effort}</span>}
                  {sug.creativeRisk && <span className={`text-[10px] px-2 py-1 rounded ${sug.creativeRisk === 'low' ? 'bg-green-900/30 text-green-400' : sug.creativeRisk === 'med' ? 'bg-yellow-900/30 text-yellow-400' : 'bg-red-900/30 text-red-400'}`}>Risk: {sug.creativeRisk}</span>}
                </div>
                <div className="text-sm text-slate-200 mb-2"><span className="text-red-400 font-medium">Issue:</span> {sug.issue}</div>
                <div className="text-sm text-slate-400 mb-2"><span className="text-green-400 font-medium">Suggestion:</span> {sug.suggestedChange}</div>
                {sug.why && <div className="text-xs text-slate-500 mb-2"><span className="text-slate-400">Why:</span> {sug.why}</div>}
                {sug.expectedCertImpact && <div className="text-xs text-indigo-400 mt-2 flex items-center gap-1"><Sparkles className="w-3 h-3" />Expected impact: {sug.expectedCertImpact}</div>}
              </div>
            ))
          )}
        </div>
...
function SeverityDots({ severity }: { severity: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <div key={i} className={`w-2 h-2 rounded-full ${i <= severity ? severity >= 4 ? 'bg-red-500' : severity >= 3 ? 'bg-orange-500' : 'bg-yellow-500' : 'bg-slate-700'}`} />
      ))}
    </div>
  )
}
