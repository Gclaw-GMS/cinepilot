...
<body>
  ...
  const handleExportMarkdown = useCallback(() => {
    ...
    // Header
    lines.push('# Character & Costume Report')
    lines.push('')
    lines.push(`**Generated by CinePilot** | ${new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}`)
    lines.push('')
    lines.push('---')
    lines.push('')
    ...
    lines.push('## Summary')
    lines.push('')
    lines.push(`- **Total Characters:** ${filteredCharacters.length}`)
    if (summary) {
      lines.push(`- **Total Budget:** ₹${summary.totalBudget.toLocaleString('en-IN')}`)
      lines.push('')
      lines.push('### By Role')
    ...
    lines.push('')
    lines.push('| Name | Age | Gender | Role | Status | Budget |')
    lines.push('|------|-----|--------|------|--------|--------|')
    ...
    for (const char of filteredCharacters) {
      ...
      lines.push(`| ${char.name} | ${char.age} | ${char.gender} | ${char.role} | ${char.status} | ${budget} |`)
    }
    lines.push('')
    lines.push('---')
    lines.push('')
    ...
    for (const char of filteredCharacters) {
      ...
      if (char.designer) lines.push(`- **Designer:** ${char.designer}`)
      if (char.estimatedBudget) lines.push(`- **Budget:** ₹${char.estimatedBudget.toLocaleString('en-IN')}`)
      if (char.description) {
        lines.push('')
      ...
      if (char.appearance.length > 0) {
        lines.push('')
        lines.push(`**Appearance:** ${char.appearance.join(', ')}`)
      }
      ...
      if (char.costumeStyle.length > 0) {
        lines.push('')
        lines.push(`**Costume Style:** ${char.costumeStyle.join(', ')}`)
      }
      if (char.fabrics.length > 0) {
        lines.push('')
        lines.push(`**Fabrics:** ${char.fabrics.join(', ')}`)
      }
      if (char.colorPalette.length > 0) {
        lines.push('')
        lines.push(`**Color Palette:** ${char.colorPalette.join(', ')}`)
      }
      if (char.costumeNotes) {
        lines.push('')
        lines.push(`**Costume Notes:**`)
        lines.push(char.costumeNotes)
      }
      lines.push('')
      lines.push('---')
      lines.push('')
  ...
  // Set up export markdown ref for keyboard shortcut
  useEffect(() => {
    handleExportMarkdownRef.current = handleExportMarkdown
  }, [handleExportMarkdown])

  // Update filtered characters ref when filtered characters change
  useEffect(() => {
