...
export default function ContinuityPage() {
  ...
  const handleExportRef = useRef<((format: 'csv' | 'json' | 'pdf' | 'markdown') => void) | null>(null);
  const filteredWarningsLengthRef = useRef(0);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'm':
          e.preventDefault();
          if (filteredWarningsLengthRef.current > 0) {
  ...
  }, [showExportMenu, showFilters, showPrintMenu, sortOrder, warnings.length]);
  ...
  const handleExport = (format: 'csv' | 'json' | 'pdf' | 'markdown') => {
    ...
    if (format === 'json') {
      ...
      filteredWarnings.forEach(w => {
        if (severityCounts[w.severity as keyof typeof severityCounts] !== undefined) {
          severityCounts[w.severity as keyof typeof severityCounts]++;
        }
        typeCounts[w.warningType] = (typeCounts[w.warningType] || 0) + 1;
      });

      const getSeverityEmoji = (severity: string) => {
        const emojis: Record<string, string> = { critical: '🔴', high: '🟠', medium: '🟡', low: '⚪' };
...
| Metric | Value |
|--------|-------|
| **Total Issues** | ${filteredWarnings.length} |
...
${Object.entries(severityCounts).map(([severity, count]) => `- ${getSeverityEmoji(severity)} ${severity.charAt(0).toUpperCase() + severity.slice(1)}: ${count}`).join('\n')}
...
| Scene | Type | Severity | Description |
|-------|------|----------|-------------|
${data.map(d => `| ${d.scene} | ${getTypeLabel(d.type)} | ${getSeverityEmoji(d.severity)} ${d.severity} | ${d.description} |`).join('\n')}

---

*Generated by CinePilot - Production Continuity Management*
`;
...
<body>
  ...
  useEffect(() => {
    ...
    filteredWarningsLengthRef.current = filteredWarnings.length;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [handleExport, filteredWarnings]);
          ...
          >
            ...
            <div className="space-y-3">
              {[
                ...
                { key: 'M', description: 'Export Markdown report' },
