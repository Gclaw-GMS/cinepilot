...
export default function DailyReportPage() {
  ...
  const exportToPDF = (report: DailyReport) => {
    const sceneStats = calculateDayStats(report)
    const completion = report.totalShotsPlanned > 0 
      ? Math.round((report.totalShotsShot / report.totalShotsPlanned) * 100) 
      : 0
...
<html>
<head>
  <title>Daily Report - Day ${report.dayNumber}</title>
  <style>
    ...
    .status-not_done { color: #ef4444; }
    .incidents { background: #fef2f2; border-left: 4px solid #ef4444; padding: 15px; margin: 20px 0; }
    .notes { background: #f8fafc; padding: 15px; border-radius: 8px; margin: 20px 0; }
    .footer { margin-top: 40px; text-align: center; color: #94a3b8; font-size: 12px; }
  </style>
</head>
<body>
  <h1>📽️ Daily Production Report - Day ${report.dayNumber}</h1>
  ...
  <div class="stats">
    <div class="stat-box">
      <div class="stat-value">${report.callTime}</div>
      <div class="stat-label">Call Time</div>
    </div>
    <div class="stat-box">
      <div class="stat-value">${report.wrapTime}</div>
      <div class="stat-label">Wrap Time</div>
    </div>
    <div class="stat-box">
      <div class="stat-value">${report.weather}</div>
    ...
    <tbody>
      ${report.scenes.map(s => `
        <tr>
          <td>#${s.sceneNumber}</td>
          <td>${s.description}</td>
          <td>${s.actualShots}/${s.plannedShots}</td>
          <td class="status-${s.status}">${s.status.charAt(0).toUpperCase() + s.status.slice(1).replace('_', ' ')}</td>
          <td>${s.notes || '-'}</td>
        </tr>
      `).join('')}
    </tbody>
  ...
  <div class="stats">
    <div class="stat-box">
      <div class="stat-value">${sceneStats.completed}</div>
      <div class="stat-label">Completed</div>
    </div>
    <div class="stat-box">
      <div class="stat-value">${sceneStats.partial}</div>
      <div class="stat-label">Partial</div>
    </div>
    <div class="stat-box">
      <div class="stat-value">${sceneStats.notDone}</div>
      <div class="stat-label">Not Done</div>
    </div>
    <div class="stat-box">
      <div class="stat-value">${report.crewPresent + report.castPresent}</div>
  ...
  <div class="incidents">
    ...
    <ul>${report.incidents.map(i => `<li>${i}</li>`).join('')}</ul>
  </div>
  ` : ''}
  ...
  <div class="footer">
    Generated by CinePilot • Daily Production Reports
  </div>
</body>
</html>`
  ...
  const generateSummaryReport = () => {
    const totalShots = reports.reduce((sum, r) => sum + r.totalShotsShot, 0)
    const plannedShots = reports.reduce((sum, r) => sum + r.totalShotsPlanned, 0)
    const totalCrew = reports.reduce((sum, r) => sum + r.crewPresent, 0)
    const totalCast = reports.reduce((sum, r) => sum + r.castPresent, 0)
    const avgCompletion = reports.length > 0 
      ? Math.round(reports.reduce((sum, r) => sum + (r.totalShotsPlanned > 0 ? (r.totalShotsShot / r.totalShotsPlanned) * 100 : 0), 0) / reports.length)
      : 0
...
<head>
  ...
  <style>
    ...
    th { background: #f1f5f9; font-weight: 600; }
    .completion-good { color: #10b981; }
    .completion-warning { color: #f59e0b; }
    .completion-bad { color: #ef4444; }
    .footer { margin-top: 40px; text-align: center; color: #94a3b8; font-size: 12px; }
  </style>
</head>
<body>
  ...
  <div class="stats">
    <div class="stat-box">
      ...
      <div class="stat-label">Total Crew Days</div>
    </div>
    <div class="stat-box">
      <div class="stat-value">${totalCast}</div>
      <div class="stat-label">Total Cast Days</div>
    </div>
    <div class="stat-box">
      <div class="stat-value">${plannedShots - totalShots}</div>
      <div class="stat-label">Remaining Shots</div>
    </div>
  </div>
  ...
  <table>
    ...
    <tbody>
      ${reports.map(r => {
        const comp = r.totalShotsPlanned > 0 ? Math.round((r.totalShotsShot / r.totalShotsPlanned) * 100) : 0
        const compClass = comp >= 90 ? 'completion-good' : comp >= 70 ? 'completion-warning' : 'completion-bad'
        return `
        <tr>
          <td>Day ${r.dayNumber}</td>
          <td>${formatDate(r.date)}</td>
          <td>${r.location}</td>
          <td>${r.totalShotsShot}/${r.totalShotsPlanned}</td>
          <td class="${compClass}">${comp}%</td>
        </tr>
      `
      }).join('')}
    </tbody>
