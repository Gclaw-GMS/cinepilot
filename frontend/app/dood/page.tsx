...
Generated by CinePilot`
  ...
  const handleExportMarkdown = useCallback(() => {
    ...
    // Calculate call frequency groups for export
    const heavyCall = report.filter(r => r.percentage >= 70)
    const moderateCall = report.filter(r => r.percentage >= 40 && r.percentage < 70)
    const lightCall = report.filter(r => r.percentage > 0 && r.percentage < 40)
    const unassigned = report.filter(r => r.percentage === 0)

    // Calculate workload warnings
    ...
    report.forEach(actor => {
      ...
      if (maxConsecutive >= 5) {
        workloadWarnings.push(`⚠️ **${actor.character}**: ${maxConsecutive} consecutive shooting days (overwork risk)`)
      }
      if (gaps.some(g => g < 2) && gaps.some(g => g < 2)) {
        workloadWarnings.push(`⚠️ **${actor.character}**: Insufficient rest days detected`)
      }
    })

    // Calculate daily cast requirements
    const dayCastCount: Record<number, string[]> = {}
    for (let d = 1; d <= stats.totalShootingDays; d++) {
      dayCastCount[d] = report.filter(r => r.days.includes(d)).map(r => r.character)
    }
    const heavyDays = Object.entries(dayCastCount).filter(([_, chars]) => chars.length > 3).length

    // Build markdown
    let markdown = `# CinePilot DOOD Report
...
| Role | Characters |
|------|------------|
| Main Cast | ${report.filter(r => r.isMain).length} |
| Supporting | ${report.filter(r => !r.isMain).length} |

## Call Frequency Groups

| Group | Count | Characters |
|-------|-------|------------|
| Heavy Call (70%+) | ${heavyCall.length} | ${heavyCall.map(r => r.character).join(', ') || '-'} |
| Moderate Call (40-70%) | ${moderateCall.length} | ${moderateCall.map(r => r.character).join(', ') || '-'} |
| Light Call (<40%) | ${lightCall.length} | ${lightCall.map(r => r.character).join(', ') || '-'} |
| Unassigned (0%) | ${unassigned.length} | ${unassigned.map(r => r.character).join(', ') || '-'} |

## Cast Schedule

| Character | Tamil Name | Actor | Total Days | Percentage | Role | Working Days |
|-----------|------------|-------|------------|------------|------|--------------|
${filteredReport.map(r => `| ${r.character} | ${r.characterTamil} | ${r.actorName || 'TBA'} | ${r.total_days} | ${r.percentage}% (${getPercentageLabel(r.percentage)}) | ${r.isMain ? 'Main Cast' : 'Supporting'} | ${r.days.join(', ')} |`).join('\n')}

## Day Distribution

| Day | Characters Working | Count |
|-----|-------------------|-------|
${Array.from({ length: stats.totalShootingDays }, (_, i) => {
  const dayNum = i + 1
  const charsWorking = report.filter(r => r.days.includes(dayNum)).map(r => r.character).join(', ')
  return `| Day ${dayNum} | ${charsWorking || '-'} | ${report.filter(r => r.days.includes(dayNum)).length} |`
}).join('\n')}

## Workload Warnings

${workloadWarnings.length > 0 ? workloadWarnings.join('\n') : '✅ No workload warnings detected - all cast members have reasonable schedules.'}

## Production Notes

- **Heavy Shoot Days:** ${heavyDays} day(s) with more than 3 cast members
- **Highest Demand:** Day ${Object.entries(dayCastCount).sort((a, b) => b[1].length - a[1].length)[0]?.[0] || '-'} (${Math.max(...Object.values(dayCastCount).map(c => c.length))} actors)
- **Lightest Day:** Day ${Object.entries(dayCastCount).sort((a, b) => a[1].length - b[1].length)[0]?.[0] || '-'} (${Math.min(...Object.values(dayCastCount).map(c => c.length))} actors)
...
`
    ...
    URL.revokeObjectURL(url)
    setShowExportMenu(false)
  }, [filteredReport, stats, selectedProject, report])

  // Update handleExportMarkdown ref
  useEffect(() => {
        ...
        {showFilters && (
          ...
          >
            ...
            <div className="flex flex-wrap gap-4">
              ...
              {activeFilterCount > 0 && (
                <button
                  onClick={() => { setFilterRole('all'); setFilterSearch(''); setSortBy('character'); setSortOrder('asc'); }}
