...
export default function Dashboard() {
  ...
  // Export dashboard stats to Markdown
  const handleExportMarkdown = useCallback(() => {
    const markdown = `# CinePilot Dashboard Report
Generated: ${new Date().toLocaleString()}

## Production Overview

### Scripts
- Total: ${stats.scripts.total}
- Scenes: ${stats.scripts.scenes}
- Characters: ${stats.scripts.characters}

### Shots
- Total: ${stats.shots.total}
- Missing Fields: ${stats.shots.missingFields}
- Runtime: ~${stats.shots.runtimeMin} minutes

### Budget
- Planned: ₹${(stats.budget.planned / 10000000).toFixed(2)}Cr
- Spent: ₹${(stats.budget.actual / 10000000).toFixed(2)}Cr
- Variance: ₹${(stats.budget.variance / 10000000).toFixed(2)}Cr

### Schedule
- Shoot Days: ${stats.schedule.days}
- Scenes Scheduled: ${stats.schedule.scenes}

### Locations
- Scenes Scouted: ${stats.locations.scenes}
- Candidates: ${stats.locations.candidates}

### Censor Board
- Certificate: ${stats.censor.certificate}
- Score: ${stats.censor.score}/100

### Storyboard
- Frames: ${stats.storyboard.frames}
- Approved: ${stats.storyboard.approved}
...
*Generated by CinePilot*`
    ...
    a.href = url
    a.download = `cinepilot-dashboard-${new Date().toISOString().split('T')[0]}.md`
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'e':
          ...
          break
        case 'm':
          if (!showKeyboardHelpRef.current && !showExportMenuRef.current) {
            e.preventDefault()
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="relative">
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm transition-colors ${showExportMenu ? 'bg-slate-700' : ''}`}
                title="Export (E)"
              >
              ...
              {showExportMenu && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-50">
                  ...
                  >
                    ...
                    <div>
                      <div className="text-sm text-slate-200">Export Markdown</div>
                      <div className="text-xs text-slate-500">Download as .md file</div>
                    </div>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <div>
                      <div className="text-sm text-slate-200">Print Report</div>
                      <div className="text-xs text-slate-500">Open print dialog</div>
                    </div>
                  </button>
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowKeyboardHelp(false)}>
          <div className="bg-slate-900 border border-white/20 rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <ShortcutRow keys={['E']} description="Open export menu" />
              <ShortcutRow keys={['M']} description="Export as Markdown" />
              <ShortcutRow keys={['P']} description="Open print dialog" />
