...
export default function ProgressPage() {
  ...
  const handleExportMarkdown = () => {
    ...
    const phasesCompleted = progress.phases.filter(p => p.status === 'completed').length
...
| Task | Status | Progress | Priority | Due Date |
|------|--------|----------|----------|----------|
${progress.tasks.map(t => `| ${t.name} | ${getStatusLabel(t.status)} | ${t.progress}% | ${getPriorityLabel(t.priority)} | ${t.dueDate || '-'} |`).join('\n')}
...
|------|----------|-----------|
${progress.upcoming_deadlines.map(d => `| ${d.task} | ${d.date} | ${d.days_left} days |`).join('\n')}
