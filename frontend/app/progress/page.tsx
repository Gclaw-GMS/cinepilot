...
export default function ProgressPage() {
  ...
  const handleExportMarkdown = useCallback(() => {
    ...
    let markdown = `# CinePilot Progress Report
...
| Metric | Value |
|--------|-------|
| Total Phases | ${progress.phases.length} |
| Phases Completed | ${progress.phases.filter(p => p.status === 'completed').length} |
| Total Milestones | ${progress.milestones.length} |
| Milestones Completed | ${progress.milestones.filter(m => m.status === 'completed').length} |
| Total Tasks | ${progress.tasks.length} |
| Tasks Completed | ${progress.tasks.filter(t => t.status === 'completed').length} |
| Tasks In Progress | ${progress.tasks.filter(t => t.status === 'in_progress').length} |
| Tasks Blocked | ${progress.tasks.filter(t => t.status === 'blocked').length} |
...
|-------|--------|----------|
${progress.phases.map(p => `| ${p.displayName} | ${getStatusLabel(p.status)} | ${p.progress}% |`).join('\n')}

---

## Milestones

| Name | Date | Status | Tasks |
|------|------|--------|-------|
${progress.milestones.map(m => `| ${m.name} | ${m.date} | ${getStatusLabel(m.status)} | ${m.tasks} |`).join('\n')}
...
|------|--------|----------|----------|----------|
${progress.tasks.map(t => `| ${t.name} | ${getStatusLabel(t.status)} | ${getPriorityLabel(t.priority)} | ${t.progress}% | ${t.dueDate || '-'} |`).join('\n')}

---

## Upcoming Deadlines

${progress.upcoming_deadlines.length > 0 ? progress.upcoming_deadlines.map(d => `- **${d.task}**: ${d.days_left} days left (${d.date})`).join('\n') : '_No upcoming deadlines_'}
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'm':
          e.preventDefault()
          if (progressRef.current) handleExportMarkdownRef.current()
          break
        case 'f':
          e.preventDefault()
