...
<body>
  ...
  const handleExportMarkdown = () => {
    ...
    const statusCounts = exportData.reduce<Record<string, number>>((acc, p) => {
      acc[p.status] = (acc[p.status] || 0) + 1
      return acc
    }, {})
    
    const languageCounts = exportData.reduce<Record<string, number>>((acc, p) => {
      if (p.language) {
        acc[p.language] = (acc[p.language] || 0) + 1
      }
      return acc
    ...
    const genreCounts = exportData.reduce<Record<string, number>>((acc, p) => {
      if (p.genre) {
...
| Average Budget | ₹${(totalBudget / exportData.length / 10000000 || 0).toFixed(2)} Cr |
| Active Production | ${statusCounts['production'] || 0} |
| In Planning | ${statusCounts['planning'] || 0} |
...
| Status | Count |
|--------|-------|
${Object.entries(statusCounts).map(([status, count]) => `| ${statusEmoji[status] || '📁'} ${status.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())} | ${count} |`).join('\n')}
...
| Language | Count |
|----------|-------|
${Object.entries(languageCounts).map(([lang, count]) => `| ${lang.charAt(0).toUpperCase() + lang.slice(1)} | ${count} |`).join('\n')}
...
| Genre | Count |
|-------|-------|
${Object.entries(genreCounts).map(([genre, count]) => `| ${genre} | ${count} |`).join('\n')}

---

## Projects Detail

| Name | Status | Language | Genre | Budget | Start | End |
|------|--------|----------|-------|--------|-------|-----|
${exportData.map((p: Project) => `| ${p.name} | ${statusEmoji[p.status] || '📁'} ${p.status.replace('_', ' ')} | ${p.language || '-'} | ${p.genre || '-'} | ${p.budget ? '₹' + (parseFloat(p.budget) / 10000000).toFixed(1) + ' Cr' : '-'} | ${p.startDate || '-'} | ${p.endDate || '-'} |`).join('\n')}

---

## Project Details

${exportData.map((p: Project) => `### ${p.name}

- **Status:** ${statusEmoji[p.status] || '📁'} ${p.status.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
- **Language:** ${p.language || 'Not specified'}
- **Genre:** ${p.genre || 'Not specified'}
- **Budget:** ${p.budget ? '₹' + parseFloat(p.budget).toLocaleString('en-IN') : 'Not specified'}
- **Start Date:** ${p.startDate || 'Not specified'}
- **End Date:** ${p.endDate || 'Not specified'}
