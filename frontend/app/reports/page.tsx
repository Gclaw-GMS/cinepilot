...
export default function ReportsPage() {
  ...
  const handleExportMarkdown = useCallback(() => {
    ...
    const markdown = `# Production Report - CinePilot
...
| Spent | ${formatCurrency(reportData.production.spent)} |
| Remaining | ${formatCurrency(reportData.production.budget - reportData.production.spent)} |
| Budget Used | ${reportData.production.budget > 0 ? ((reportData.production.spent / reportData.production.budget) * 100).toFixed(1) : 0}% |
...
| Completion | ${reportData.schedule.totalScenes > 0 ? ((reportData.schedule.scenesShot / reportData.schedule.totalScenes) * 100).toFixed(1) : 0}% |
...
| Metric | Value |
|--------|-------|
| Predicted Certificate | ${reportData.censor.certificate} |
...
| Source | Records |
|--------|---------|
| Scripts | ${dataSources?.scripts || 0} |
| Characters | ${dataSources?.characters || 0} |
| Locations | ${dataSources?.locations || 0} |
| Shooting Days | ${dataSources?.shootingDays || 0} |
| Crew | ${dataSources?.crew || 0} |
| Expenses | ${dataSources?.expenses || 0} |
...
`
    ...
    a.click()
    setExporting(false)
  }, [reportData, dataSources, setExporting, setShowExportMenu])

  // Ref for Markdown export to use in keyboard shortcuts
