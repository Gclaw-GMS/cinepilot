...
export default function ShotHubPage() {
  ...
  const filteredShotsRef = useRef<ShotData[]>([])
  ...
  const handleExportShots = async (format: 'json' | 'csv' | 'markdown' = 'json') => {
    setExporting(true)
    try {
      ...
      if (format === 'csv') {
        ...
        const filteredShots = filteredShotsRef.current.length > 0 ? filteredShotsRef.current : shots
        
        const markdown = `# Shot List Report

**Generated:** ${new Date().toLocaleString()}
**Total Shots:** ${filteredShots.length}
**Total Duration:** ${Math.round(filteredShots.reduce((acc, s) => acc + (s.durationEstSec || 0), 0) / 60)} min
...
|--------|-------|
| Total Shots | ${filteredShots.length} |
| Total Duration | ${filteredShots.reduce((acc, s) => acc + (s.durationEstSec || 0), 0)} sec |
| Missing Fields | ${filteredShots.filter(s => !s.shotSize || !s.cameraAngle || !s.cameraMovement || !s.durationEstSec).length} |
| Avg Duration/Shot | ${filteredShots.length ? Math.round(filteredShots.reduce((acc, s) => acc + (s.durationEstSec || 0), 0) / filteredShots.length) : 0} sec |

---

## Shot Breakdown by Size

| Shot Size | Count |
|-----------|-------|
${Object.entries(filteredShots.reduce((acc, s) => { acc[s.shotSize || 'Unknown'] = (acc[s.shotSize || 'Unknown'] || 0) + 1; return acc }, {} as Record<string, number>)).map(([size, count]) => `| ${size} | ${count} |`).join('\n')}

---

## Shot Breakdown by Movement

| Camera Movement | Count |
|-----------------|-------|
${Object.entries(filteredShots.reduce((acc, s) => { acc[s.cameraMovement || 'Unknown'] = (acc[s.cameraMovement || 'Unknown'] || 0) + 1; return acc }, {} as Record<string, number>)).map(([movement, count]) => `| ${movement} | ${count} |`).join('\n')}

---

## Complete Shot List

| # | Scene | Shot | Size | Angle | Movement | Duration | Characters |
|---|-------|------|------|-------|----------|----------|------------|
${filteredShots.map(s => `| ${s.shotIndex} | ${s.scene?.sceneNumber || '-'} | ${s.shotText.substring(0, 30)}${s.shotText.length > 30 ? '...' : ''} | ${s.shotSize || '-'} | ${s.cameraAngle || '-'} | ${s.cameraMovement || '-'} | ${s.durationEstSec || '-'}s | ${s.characters?.join(', ') || '-'}|`).join('\n')}

---

*Generated by CinePilot - AI Pre-Production Assistant*
`
        const blob = new Blob([markdown], { type: 'text/markdown' })
        const downloadUrl = URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = downloadUrl
        a.download = `shot_list_${new Date().toISOString().split('T')[0]}.md`
        document.body.appendChild(a)
        a.click()
        document.body.removeChild(a)
        URL.revokeObjectURL(downloadUrl)
        setSaveMessage({ type: 'success', text: 'Exported to Markdown' })
      } else {
        const data = await res.json()
...
<body>
  ...
  // Update filtered shots ref when filtered shots change
  useEffect(() => {
    filteredShotsRef.current = filteredShots
  }, [filteredShots])

  // Update export markdown ref when handleExportShots changes
  useEffect(() => {
    handleExportMarkdownRef.current = () => handleExportShots('markdown')
  }, [handleExportShots, shots, filteredShots])
          ...
          <div className="relative" ref={exportMenuRef}>
            ...
            {showExportMenu && (
              <div className="absolute right-0 mt-1 w-36 bg-gray-800 border border-gray-700 rounded shadow-lg z-10">
                ...
                <button
                  onClick={() => { handleExportShots('markdown'); setShowExportMenu(false) }}
