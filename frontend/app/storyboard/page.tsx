...
export default function StoryboardPage() {
  ...
  const handleExportMarkdown = useCallback(() => {
    ...
    const totalFrames = filteredScenes.reduce((sum, s) => sum + s.frames.length, 0)
    const approvedCount = filteredScenes.reduce((sum, s) => sum + s.frames.filter(f => f.isApproved).length, 0)
    const pendingCount = filteredScenes.reduce((sum, s) => sum + s.frames.filter(f => f.status === 'pending').length, 0)
    const failedCount = filteredScenes.reduce((sum, s) => sum + s.frames.filter(f => f.status === 'failed').length, 0)
    const inProgressCount = filteredScenes.reduce((sum, s) => sum + s.frames.filter(f => f.status === 'generating').length, 0)

    // Group frames by status
    const statusCounts = { approved: approvedCount, pending: pendingCount, failed: failedCount, generating: inProgressCount }

    // Group frames by scene
    const sceneStats = filteredScenes.map(scene => ({
      sceneNumber: scene.sceneNumber,
      heading: scene.heading,
      totalFrames: scene.frames.length,
      approved: scene.frames.filter(f => f.isApproved).length,
      pending: scene.frames.filter(f => f.status === 'pending').length,
    }))

    // Style breakdown
    const styleCounts = filteredScenes.reduce<Record<string, number>>((acc, scene) => {
      scene.frames.forEach(frame => {
        const style = frame.style || 'unknown'
        acc[style] = (acc[style] || 0) + 1
      })
      return acc
    }, {})

    const getStatusEmoji = (status: string, isApproved: boolean) => {
    ...
    const getStyleLabel = (style: string) => {
      const labels: Record<string, string> = {
        cleanLineArt: 'Clean Line Art',
        pencilSketch: 'Pencil Sketch',
        markerLine: 'Marker & Ink',
        blueprint: 'Blueprint',
      }
...
**Script ID:** ${selectedScript || 'All Scripts'}
...
| Failed | ${failedCount} |
| Approval Rate | ${totalFrames > 0 ? Math.round((approvedCount / totalFrames) * 100) : 0}% |
...
${Object.entries(styleCounts).map(([style, count]) => `- **${getStyleLabel(style)}:** ${count} frame${count > 1 ? 's' : ''}`).join('\n')}

---

## Scene Breakdown

| Scene | Heading | Frames | Approved | Pending |
|-------|---------|--------|----------|---------|
${sceneStats.map(s => `| ${s.sceneNumber} | ${s.heading || 'N/A'} | ${s.totalFrames} | ${s.approved} | ${s.pending} |`).join('\n')}

---

## Frame Details

| Scene | Shot | Status | Style | Notes |
|-------|------|--------|-------|-------|
${filteredScenes.flatMap(scene => 
  scene.frames.map(frame => 
    `| ${scene.sceneNumber} | ${frame.shot.shotIndex} | ${getStatusEmoji(frame.status, frame.isApproved)} ${frame.isApproved ? 'Approved' : frame.status} | ${getStyleLabel(frame.style)} | ${frame.directorNotes || frame.prompt?.substring(0, 50) || '-'} |`
  )
).join('\n')}

---

*Generated by CinePilot - Production Storyboard Management*
`
