...
export default function StoryboardPage() {
  ...
  // Ref for filtered scenes to use in export functions (initialized after filteredScenes is defined)
  const filteredScenesRef = useRef<typeof scenes>(scenes)
  useEffect(() => {
    filteredScenesRef.current = scenes
  }, [scenes])

  const handleExportMarkdown = useCallback(() => {
    const scenes = filteredScenesRef.current
    if (scenes.length === 0) return
    
    const totalFrames = scenes.reduce((sum, s) => sum + s.frames.length, 0)
    const approvedFrames = scenes.reduce((sum, s) => sum + s.frames.filter(f => f.isApproved).length, 0)
    const pendingFrames = scenes.reduce((sum, s) => sum + s.frames.filter(f => f.status === 'pending').length, 0)
    const failedFrames = scenes.reduce((sum, s) => sum + s.frames.filter(f => f.status === 'failed').length, 0)
    const generatingFrames = scenes.reduce((sum, s) => sum + s.frames.filter(f => f.status === 'generating').length, 0)
    
    // Group frames by scene for breakdown
    const sceneBreakdown = scenes.map(scene => ({
      scene: scene.sceneNumber,
      frames: scene.frames.length,
      approved: scene.frames.filter(f => f.isApproved).length,
    }))
    
    let markdown = `# 🎬 CinePilot - Storyboard Report
...
### Status Breakdown
- ✅ Approved: ${approvedFrames} (${totalFrames > 0 ? ((approvedFrames / totalFrames) * 100).toFixed(1) : 0}%)
- ⏳ Pending: ${pendingFrames}
- 🔄 Generating: ${generatingFrames}
- ❌ Failed: ${failedFrames}
...
| Scene | Panels | Approved |
|-------|--------|----------|
${sceneBreakdown.map(s => `| ${s.scene} | ${s.frames} | ${s.approved} |`).join('\n')}
...
`
    ...
    // Add detailed frame information grouped by scene
    scenes.forEach(scene => {
      markdown += `### Scene ${scene.sceneNumber}\n\n`
      markdown += `| Shot | Frame ID | Status | Approved | Style |\n`
      markdown += `|------|----------|--------|----------|-------|\n`
      
      scene.frames.forEach(frame => {
        const statusIcon = frame.status === 'completed' ? '✅' : frame.status === 'generating' ? '🔄' : frame.status === 'failed' ? '❌' : '⏳'
        const approvedText = frame.isApproved ? 'Yes' : 'No'
        markdown += `| ${frame.shot.shotIndex} | ${frame.id.substring(0, 8)} | ${statusIcon} ${frame.status} | ${approvedText} | ${frame.style || '-'} |\n`
      })
      markdown += '\n'
    })
    
    // Add filter info if active
    if (searchQuery || statusFilter !== 'all' || sceneFilter !== 'all') {
      markdown += `---\n\n**Active Filters:**\n`
      if (searchQuery) markdown += `- Search: "${searchQuery}"\n`
      if (statusFilter !== 'all') markdown += `- Status: ${statusFilter}\n`
      if (sceneFilter !== 'all') markdown += `- Scene: ${sceneFilter}\n`
    }
    
    markdown += `\n---\n*Generated by CinePilot - Production Storyboard*\n`
    ...
    URL.revokeObjectURL(url)
    setShowExportMenu(false)
  }, [searchQuery, statusFilter, sceneFilter])
  ...
  const handleExportJSON = useCallback(() => {
    ...
    URL.revokeObjectURL(url)
    setShowExportMenu(false)
  }, [searchQuery, statusFilter, sceneFilter, sortBy, sortOrder])
