...
export default function TravelExpensesPage() {
  ...
  const handleExportMarkdown = useCallback(() => {
    ...
    const statusSummary = STATUS_OPTIONS.map(s => ({
      name: s.label,
      count: expenses.filter(e => e.status === s.key).length,
      total: expenses.filter(e => e.status === s.key).reduce((sum, e) => sum + e.amount, 0)
    })).filter(s => s.count > 0)
...
|----------|-------|-------|
${categorySummary.map(c => `| ${c.name} | ${c.count} | ${formatCurrency(c.total)} |`).join('\n')}
...
| Status | Count | Total |
|--------|-------|-------|
${statusSummary.map(s => `| ${s.name} | ${s.count} | ${formatCurrency(s.total)} |`).join('\n')}

---

## All Expenses

| Date | Person | Category | Description | Amount | Status |
|------|--------|----------|-------------|--------|--------|
${filteredExpenses.map(e => `| ${e.date} | ${e.personName || '-'} | ${EXPENSE_CATEGORIES.find(c => c.key === e.category)?.label || e.category} | ${e.description} | ${formatCurrency(e.amount)} | ${STATUS_OPTIONS.find(s => s.key === e.status)?.label || e.status} |`).join('\n')}
...
`
    ...
    URL.revokeObjectURL(url)
    setShowExportMenu(false)
  }, [expenses, filteredExpenses, totalExpenses, pendingExpenses, approvedExpenses, reimbursedExpenses])
...
<script>window.onload=function(){window.print()}</script></body></html>`
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'm' && !e.metaKey && !e.ctrlKey && !e.altKey) { e.preventDefault(); handleExportMarkdownRef.current?.() }
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex bg-slate-700 rounded-lg p-1">
                ...
                <button onClick={() => setViewMode('conflicts')} className={`px-3 py-1.5 rounded-md text-sm font-medium transition flex items-center gap-1 ${viewMode === 'conflicts' ? 'bg-amber-500 text-slate-900' : 'text-slate-300 hover:text-white'}`}><AlertCircle className="w-4 h-4" />Conflicts {conflictStats.total > 0 && <span className={`ml-1 px-1.5 py-0.5 text-xs rounded-full ${conflictStats.high > 0 ? 'bg-red-500 text-white' : 'bg-amber-500 text-slate-900'}`}>{conflictStats.total}</span>}</button>
              </div>
