...
export default function WhatsAppPage() {
  ...
  const handleDeleteMessage = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/whatsapp?messageId=${id}`, { method: 'DELETE' })
      const data = await res.json()
      if (data.success) {
  ...
  const handleClearAllHistory = async () => {
    ...
    try {
      const res = await fetch(`/api/whatsapp?action=clear-all`, { method: 'DELETE' })
      const data = await res.json()
      if (data.success) {
        setMessages([])
      }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {activeTab === 'history' && (<div className="space-y-4"><div className="flex items-center justify-between"><div className="flex gap-2"><div className="relative"><Search className="w-4 h-4 text-gray-500 absolute left-3 top-1/2 -translate-y-1/2" /><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search..." className="pl-9 pr-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white w-48" /></div><select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm"><option value="all">All</option><option value="schedule">Schedule</option><option value="reminder">Reminder</option><option value="call_sheet">Call Sheet</option></select></div><div className="flex gap-2"><button onClick={handleClearAllHistory} disabled={messages.length === 0} className="px-3 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-lg text-sm disabled:opacity-50"><Trash2 className="w-4 h-4 inline mr-1" />Clear All</button><button onClick={handleExportHistory} className="px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-gray-300"><Download className="w-4 h-4 inline" />Export</button></div></div><div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden"><table className="w-full"><thead className="bg-gray-800/50"><tr><th className="text-left p-4 text-xs text-gray-400">Recipient</th><th className="text-left p-4 text-xs text-gray-400">Message</th><th className="text-center p-4 text-xs text-gray-400">Status</th><th className="text-right p-4 text-xs text-gray-400">Time</th><th className="text-center p-4 text-xs text-gray-400">Actions</th></tr></thead><tbody className="divide-y divide-gray-800">{filteredMessages.map(m => (<tr key={m.id} className="hover:bg-gray-800/30"><td className="p-4"><p className="text-white">{m.recipientName || m.recipient}</p><p className="text-gray-500 text-xs">{m.recipient}</p></td><td className="p-4"><p className="text-gray-300 text-sm line-clamp-1">{m.message}</p></td><td className="p-4 text-center"><span className={`px-2 py-1 rounded-full text-xs ${STATUS_COLORS[m.status] || 'bg-gray-800 text-gray-400'}`}>{m.status}</span></td><td className="p-4 text-right text-gray-500 text-sm">{formatTime(m.timestamp)}</td><td className="p-4 text-center"><button onClick={() => handleDeleteMessage(m.id)} className="p-2 text-gray-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors" title="Delete"><Trash2 className="w-4 h-4" /></button></td></tr>))}</tbody></table></div></div>)}
