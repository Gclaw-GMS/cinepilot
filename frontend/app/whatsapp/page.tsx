...
<body>
  ...
  const handleExportMarkdown = useCallback(() => {
    ...
    // Build markdown
    let markdown = `# CinePilot WhatsApp Broadcast Report
...
`
    ...
    templates.forEach(t => {
      markdown += `| ${t.name} | ${t.category} | ${t.variables.join(', ')} | ${new Date(t.createdAt).toLocaleDateString('en-IN')} |\n`
    })
...
`
    ...
    contacts.forEach(c => {
      markdown += `| ${c.name} | ${c.phone} | ${c.role || '-'} |\n`
    })
...
| Recipient | Message | Status | Time |
|-----------|---------|--------|------|
`
    
    messages.forEach(m => {
      const shortMessage = m.message.length > 50 ? m.message.substring(0, 50) + '...' : m.message.replace(/\n/g, ' ')
      markdown += `| ${m.recipientName || m.recipient} | ${shortMessage} | ${getStatusEmoji(m.status)} ${m.status} | ${formatTimestamp(m.timestamp)} |\n`
    })
...
- **Exported From:** CinePilot WhatsApp Broadcast
- **Export Date:** ${new Date().toISOString()}
- **Total Records:** ${messages.length} messages, ${templates.length} templates, ${contacts.length} contacts
...
`
    ...
    a.download = `whatsapp-broadcast-${new Date().toISOString().split('T')[0]}.md`
    a.click()
    URL.revokeObjectURL(a.href)
    setShowExportDropdown(false)
  }, [messages, templates, contacts, isDemoMode])
